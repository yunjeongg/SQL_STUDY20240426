...
SELECT TO_CHAR(hire_date, 'MM-DD') FROM employees;

-- 5. departments 테이블에서 모든 부서의 department_name의 앞 두 글자만 출력하세요.
SELECT SUBSTR(department_name, 1, 2) FROM departments;
...
SELECT CEIL(salary/100)*100, salary FROM employees;
...
SELECT CONCAT(SUBSTR(first_name, 1, 1), SUBSTR(last_name, 1, 1)) AS initials FROM employees;
...
SELECT SUBSTR(country_name, 1, 3) FROM countries;

-- 16. employees 테이블에서 각 사원의 last_name의 두 번째 글자를 '*'로 변경하여 출력하세요
SELECT SUBSTR(last_name, 1, 1) || '*' || SUBSTR(last_name, 3) FROM employees;
